...
import { TransferIntent, SubscriptionTier, TokenType } from './types';
...
function App() {
  ...
  const totalPortfolioValue = MOCK_WALLETS.reduce((acc, w) => {
    return acc + w.balances.reduce((bAcc, b) => bAcc + (b.amount * b.priceUsd), 0);
  }, 0);
  ...
  const handleAiSubmit = async (e: React.FormEvent) => {
    ...
    if (intent) {
      ...
      setAiCommand('');
        ...
        setNotification("Couldn't understand that command. Try 'Send 50 USDC from Main to Project A'");
              ...
              return (
                <div className="space-y-6">
                    <div className="mb-6">
                        ...
                        <p className="text-slate-400">Directly transfer capital to vetted projects with transparent gas fees.</p>
                    </div>
                ...
                <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-6">
                            <div className="flex justify-between items-end mb-6">
                                <div>
                                    <h2 className="text-slate-400 text-sm font-medium mb-1">Total Portfolio Value</h2>
                                    <div className="text-4xl font-bold text-white tracking-tight">
                                        ${totalPortfolioValue.toLocaleString()}
                                    </div>
                                </div>
                                <div className="text-emerald-400 text-sm font-medium flex items-center">
                                    +4.25% <span className="text-slate-500 ml-1">past 24h</span>
                                </div>
                            </div>
                        ...
                        <div className="bg-gradient-to-br from-slate-900 to-indigo-900/20 border border-slate-800 rounded-2xl p-6 flex flex-col justify-center">
                            ...
                            <div className="space-y-3">
                                <button onClick={() => setShowTransfer(true)} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-xl font-medium transition-colors text-left px-4 flex justify-between items-center group shadow-lg shadow-indigo-500/20">
                                ...
                                <button onClick={() => setActiveTab('buysell')} className="w-full bg-slate-800 hover:bg-slate-700 text-slate-200 py-3 rounded-xl font-medium transition-colors text-left px-4 flex justify-between items-center">
                                    <div className="flex items-center gap-3">
                                        <span className="p-1 bg-white/10 rounded">🏦</span>
                                        <span>Buy Crypto (Soon)</span>
                                    </div>
                                </button>
                                <button onClick={() => setActiveTab('bridge')} className="w-full bg-slate-800 hover:bg-slate-700 text-slate-200 py-3 rounded-xl font-medium transition-colors text-left px-4 flex justify-between items-center">
                                    <div className="flex items-center gap-3">
                                        <span className="p-1 bg-white/10 rounded">🌉</span>
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={() => setActiveTab('settings')} className="text-sm text-indigo-400 hover:text-white">Manage Preferences</button>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-indigo-500/30 flex">
      ...
      <div className={`flex-1 transition-all duration-300 ${isSidebarOpen ? 'lg:ml-64' : 'lg:ml-0'} relative`}>
         <main className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 ${!isSidebarOpen ? 'pt-20 lg:pt-8' : ''}`}>
            
            {/* Header / Command Bar */}
            <div className="mb-8 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                <div className={`flex items-center gap-4 transition-all ${!isSidebarOpen ? 'pl-12 lg:pl-0' : ''}`}>
